...
final class STAnnotationSelectionView: NSView {
    ...
    var onAnnotationSelected: ((_ annotation: PDFAnnotation, _ page: PDFPage) -> Void)?
    var onAnnotationMoved: ((_ annotation: PDFAnnotation, _ page: PDFPage, _ oldBounds: CGRect) -> Void)?
    var onAnnotationResized: ((_ annotation: PDFAnnotation, _ page: PDFPage, _ oldBounds: CGRect) -> Void)?
    var onAnnotationTapped: ((_ annotation: PDFAnnotation, _ page: PDFPage) -> Void)?
    var onSelectionCleared: (() -> Void)?
    var onSelectionAction: ((_ action: STSelectionAction) -> Void)?
    var onMultiAnnotationSelected: ((_ annotations: [PDFAnnotation], _ page: PDFPage) -> Void)?
    ...
    func updateSelection() {
        ...
        let localBounds = convert(viewBounds, from: pdfView)
        showHandles(around: localBounds)
    }
    ...
    private func showHandles(around rect: CGRect) {
        for position in STHandlePosition.allCases {
            let handleLayer = CALayer()
            handleLayer.backgroundColor = NSColor.systemBlue.cgColor
            handleLayer.borderColor = NSColor.white.cgColor
            handleLayer.borderWidth = 1
            handleLayer.cornerRadius = handleSize / 2
            let offset = position.offset
            let center = CGPoint(x: rect.minX + rect.width * offset.x, y: rect.minY + rect.height * offset.y)
            handleLayer.frame = CGRect(x: center.x - handleSize/2, y: center.y - handleSize/2, width: handleSize, height: handleSize)
            layer?.addSublayer(handleLayer)
            handles.append(handleLayer)
        }
    }

    private func clearHandles() {
        handles.forEach { $0.removeFromSuperlayer() }
        handles.removeAll()
    }
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        guard let pdfView = pdfView else { return }
        let pointInPDFView = convert(pt, to: pdfView)
        ...
        let pointInPage = pdfView.convert(pointInPDFView, to: page)
        for annotation in page.annotations.reversed() {
            if let subtype = annotation.type, Self.protectedSubtypes.contains(subtype) { continue }
    ...
    override func mouseDragged(with event: NSEvent) {
        guard let pdfView = pdfView,
              ...
              let start = dragStart,
              let origBounds = originalBounds else { return }
        let pt = convert(event.locationInWindow, from: nil)
        let dx = pt.x - start.x
        let dy = pt.y - start.y
        ...
        annotation.bounds = CGRect(x: origBounds.origin.x + pageDx, y: origBounds.origin.y + pageDy,
    ...
    override func mouseUp(with event: NSEvent) {
        guard let annotation = annotationManager?.selectedAnnotation,
              let page = annotationManager?.selectedAnnotationPage,
              let origBounds = originalBounds else { return }
        if annotation.bounds != origBounds {
            onAnnotationMoved?(annotation, page, origBounds)
        }
        dragStart = nil
        originalBounds = nil
        updateSelection()
