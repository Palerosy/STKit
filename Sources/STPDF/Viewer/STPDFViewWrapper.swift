...
struct STPDFViewWrapper: UIViewRepresentable {
    ...
    func updateUIView(_ container: UIView, context: Context) {
        ...
        for overlay in overlayViews {
            if !overlay.isHidden {
                ...
                let mask = CAShapeLayer(); mask.path = combinedPath.cgPath; overlay.layer.mask = mask
            } else {
    ...
    class Coordinator: NSObject {
        ...
        init(parent: STPDFViewWrapper) { self.parent = parent; self.annotationManager = parent.annotationManager }
        ...
        @objc func viewChanged(_ notification: Notification) {
            DispatchQueue.main.async {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in self?.selectionView?.refreshVisuals() }
            }
        }
...
struct STPDFViewWrapper: NSViewRepresentable {
    ...
    func updateNSView(_ container: NSView, context: Context) {
        ...
        let isMarkup = tool?.requiresTextSelection == true
        // macOS has no isUserInteractionEnabled — hide overlays when not active to prevent event interception
        drawingView.isHidden = !isDrawing
        if isMarkup || !isDrawing { drawingView.clearOverlayLayers() }
        if !isDrawing { drawingView.clearCurrentStroke() }
